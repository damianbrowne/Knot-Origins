...
if (isset($_POST['login-submit'])) {
  ...
  if (empty($emailusername) || empty($password)) {
    header("Location: ../index.php?error=emptyfields&emailusername=".$emailusername);
    exit();
  }
  else {
    ...
    // Here we initialize a new statement using the connection from the connect.php file.
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
      ...
      if ($row = mysqli_fetch_assoc($result)) {
        ...
        // Then if they DO match, then we know it is the correct user that is trying to log in!
          ...
          setcookie("username", $_SESSION['username'], time()+(10 * 365 * 24 * 60 * 60),'/',NULL,NULL,false);
