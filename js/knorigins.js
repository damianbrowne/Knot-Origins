...
function _key_listener_Player_1 (event) {
  ...
  switch (event.key) {
    ...
    case "p": player.facing = {r: -1, c: 0}; player.useAbility(); break;
/*    let numOfTests = 0;
...
*/
    case "l": player.facing = {r: 0, c: -1}; player.useAbility(); break;
    case ";": player.facing = {r: 1, c: 0}; player.useAbility(); break;
    case "'": player.facing = {r: 0, c: 1}; player.useAbility(); break;
...
function _key_listener_Player_2 (event) {
  ...
  switch (event.key) {
    ...
    case "t": player2.facing = {r: -1, c: 0}; player2.useAbility(); break;
    case "f": player2.facing = {r: 0, c: -1}; player2.useAbility(); break;
    case "g": player2.facing = {r: 1, c: 0}; player2.useAbility(); break;
    case "h": player2.facing = {r: 0, c: 1}; player2.useAbility(); break;
  //  case " ": activeGame.player.useAbility(); return
  }
...
configButt.onclick = function () {
  let totalPlayers = document.querySelector(playerSelect).value,
      ...
      maze,
      mazeSelectorVar = Math.floor(Math.random()*3);
  console.log(difficulty);
  console.log(mazeSelectorVar);
  switch (difficulty) {
    case "easy": maze = possibleMazes[mazeSelectorVar]; console.log(maze); break;
    case "medium": maze = possibleMazes[mazeSelectorVar + 3]; console.log(maze); break;
    case "hard": maze = possibleMazes[mazeSelectorVar + 6]; console.log(maze); break;
  }
  console.log(maze);
  if (!isValidMaze(maze)) {
...
class Knotbject {
  ...
  //      this.game.eraseAt(this, this.r, this.c);
   ...
   //      this.game.eraseAt(this, this.r, this.c);
         switch (playerType[0].asset) {
           case this.game.player.asset:
            this.game.score += (Math.min(100, this.game.cureProgress + 25) - this.game.cureProgress) * this.game.scoreMultiplier //originally +30
            this.game.cureProgress = Math.min(100, this.game.cureProgress + 25) //originally +30
            updateScore(this.game.score);
...
//    updateRound(this.round);
  ...
  setPause () {
    message = "Game Paused HELLO WORLD";
    removePlayer1Keys();
    removePlayer2Keys();
    //this.ticking = 0;
  ...
  doTick () {
    ...
    this.score += (-1/9) * this.score + (10/9) * this.scoreMultiplier
    if (this.cureProgress >= 100) {
      this.win();
    } else if (this.cureProgress > 0) { // else if (this.round % 2 === 0 || this.round % 2 === 1)
      this.cureProgress -= this.diffMultiplier * (5/8) //originally (2/3) --ended in about 7 rounds min, then 4/9--ended in about 12 rounds min
      this.score -= this.diffMultiplier * (5/8) * this.scoreMultiplier //originally (2/3) ||
...
/*
  ...
  */
   ...
   */
    ...
    form.querySelectorAll("input [name = 'final-score']").value = Math.floor(this.score) ;
    */
    form.submit();
/*
    document.getElementById('username').value = username
    document.getElementById('final-score').value = Math.floor(this.score)
*/
  ...
  win () {
    ...
    let username = prompt(`
      You did it! \n
      You found the cure after ${this.round} rounds. \n
      Score: ${Math.floor(this.score)} \n
      Username:
      `);
...
function isValidMaze (maze) {
  ...
  let isAllXRow = function (row) {
      ...
      zombieCount = 0;
      console.log(maze);
      let columnCount = maze[0].length; //maze[0] &&
      ...
      <div id="lobby-container">
        ...
        <div id="lobby-options">
          ...
          <div id="character-grid" class='fant-font'>
            <input type='radio' name='diff-select' value='ktrivial' id="diff-trivial"/><label for="diff-trivial">I</label>
            <input type='radio' name='diff-select' value='ktolerable' id="diff-tolerable" checked="checked"/><label for="diff-tolerable">II</label>
            <input type='radio' name='diff-select' value='kterrible' id="diff-terrible"/><label for="diff-terrible">III</label>
          </div>
          <br/>
          <hr/>
          <br/>
